...
func Test_CleanupCmd(t *testing.T) {
	...
	t.Run("doMarkOne", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("doCleanupOne", func(t *testing.T) {
		t.Parallel()
		t.Run("done", func(t *testing.T) {
			t.Parallel()
			p := setup(t)
			...
			err := doCleanupOne(p.ctx, p.dc, p.di, p.projectID, p.zone, p.dryRun)
			require.EqualError(t, err, iterator.Done.Error())
		})
		...
		t.Run("disk missing label", func(t *testing.T) {
			t.Parallel()
			p := setup(t)
